...
/// # Grid main implementation
impl Grid {
    /// # Create a grid
    /// Need a size for creating a square grid and default fill state.
    pub fn new(kind: GridKind, size: usize, default_state: bool) -> Grid {
    ...
    pub fn prepare_tiles(tiles_in: Vec<Tile>) -> Vec<Tile> {
    ...
    /// Enter x and y to get tile state. 
    pub fn state_tile(self: &Self, x: i32, y:i32) -> TileState {
        let mut return_state: TileState = TileState::Void;
        let mut i: usize = 0;
        while return_state == TileState::Void && i < self.tiles.len() {
            let tile: &Tile = &self.tiles[i]; 
            if tile.x == x && tile.y == y {
                if tile.state == true {
                    return_state = TileState::On;
                } else {
                    return_state = TileState::Off;
                }
            }
            i += 1;
        }
    ...
    pub fn update_tile(self: &mut Self, x: i32, y:i32, state: bool, features_new: Vec<TileFeatures>) {
        let mut grid_updated: Grid = self.clone();
        for tile in &self.tiles {
            if tile.x == x && tile.y == y {
                let tile_index: usize = self.tiles.iter().position(|t| t == tile).expect("(X) - Tile somehow not found.");
    ...
    pub fn display_inline(self: &Self, local_ui_tiles: &UiTiles, local_ui_features: &HashMap<TileFeatures, &'static str>) {
        // Find maximums
        let mut max_x: i32 = 0;
        let mut min_x: i32 = 0;
        let mut max_y: i32 = 0;
